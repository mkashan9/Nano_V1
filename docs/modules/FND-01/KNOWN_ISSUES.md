...
5. Melos global vs project version may differ (7.x workspace vs 8.x global) — always use `dart run melos` from repo root (or `.\scripts\bootstrap.ps1`); do not rely on a global `melos` PATH entry.
