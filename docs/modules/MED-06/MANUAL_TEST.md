...
### 2. Record one line in the cast voice
...
- `voice_id = guide_educational`
- Learner catalog still empty for that slot

The default voice is Fish Educational Guide
(`2c408095b1294de896376eff6a638d90`), chosen by the owner and recorded in
ADR-0008. Stock is no longer the default.
...
### 4. Compose one clip (json2video path)
...
The owner later rejected this clip as looking fake; character animation via
image-to-video is MED-07. The compose gate and provenance still matter and are
...
### 6. Hear the cast line

Open admin_web → Moderation and select the `narration_greeting-2` MP3. It plays
in place through the browser's audio element. Decide it on what you hear.

The stock recording and the json2video clip were already rejected with the
owner's reasons recorded. Approving the cast voice is the remaining decision
that publishes something to a learner.
...
- One Fish MP3 in the cast Educational Guide voice sits unreviewed in the queue
...
- The guide voice is `guide_educational`, not stock
